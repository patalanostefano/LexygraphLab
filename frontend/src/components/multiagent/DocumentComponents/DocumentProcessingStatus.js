...
export const DocumentProcessingStatus = ({ document, onCancel }) => {
  ...
  const getFileIcon = () => {
    ...
    if (document.type.includes('pdf')) return <PictureAsPdfIcon fontSize="small" color="error" />;
    if (document.type.includes('word') || document.type.includes('document')) return <ArticleIcon fontSize="small" color="primary" />;
    if (document.type.includes('image')) return <ImageIcon fontSize="small" color="success" />;
    ...
    >
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          {getFileIcon()}
          <Typography variant="body2" sx={{ ml: 1, fontWeight: 500, maxWidth: '190px' }} noWrap>
            {document.name}
          </Typography>
        </Box>
