...
import { EditProjectDialog, DeleteConfirmDialog } from '../';
...
function Multiagent() {
  ...
  useEffect(() => {
    const loadUserProjects = async () => {
      try {
        ...
        if (response.collections) {
          ...
          const projectsFromCollections =
            response.collections.map(collectionToProject);
          ...
          const projectsWithDocuments = await Promise.all(
            projectsFromCollections.map(async (project) => {
              try {
                const docsResponse =
                  await collectionsApi.getCollectionDocuments(
                    project.collectionId,
                  );

                // MODIFICATO: Marca tutti i documenti caricati dal backend come "uploaded"
                const markedDocuments = (docsResponse.documents || []).map(
                  (doc) => markDocumentAsUploaded(doc),
                );
                ...
                console.error(
                  `Error loading documents for collection ${project.collectionId}:`,
                  error,
                );
                return project;
              }
          ...
          localStorage.setItem(
            storageKey,
            JSON.stringify(projectsWithDocuments),
          );
        }
      } catch (error) {
    ...
    setMessages([
      {
        type: 'user',
        content: 'Puoi prepararmi un contratto di locazione?',
        timestamp: new Date(),
        agent: null,
      },
      {
        type: 'agent',
        content:
          "Certo, posso aiutarti a preparare un contratto di locazione. Per procedere, avrei bisogno di alcune informazioni essenziali:\n\n1. Dati del locatore (proprietario)\n2. Dati del conduttore (inquilino)\n3. Indirizzo dell'immobile\n4. Durata del contratto\n5. Importo del canone mensile\n6. Modalità di pagamento\n7. Deposito cauzionale\n\nVuoi fornirmi queste informazioni o preferisci che ti mostri un modello standard che potrai personalizzare in seguito?",
        timestamp: new Date(),
        agent: 'assistente',
      },
    ]);
  ...
  const handleSelectProject = (project) => {
    try {
      ...
      if (!project || !project.id) {
        throw new Error(
          'Progetto non valido o mancante di attributi essenziali',
        );
      }
      ...
      if (project.documents && project.documents.length > 0) {
        const generatedDocs = project.documents.filter(
          (doc) => doc.isGenerated || doc.generatedBy,
        );
        ...
        for (const doc of newProject.documents) {
          if (doc.file) {
            ...
            try {
              ...
              await collectionsApi.addDocumentToCollection(
                collection.id,
                doc.id,
              );
              // MODIFICATO: Marca come caricato se non ha già i flag
              const markedDoc = doc.isGenerated
                ? doc
  ...
  const handleUpdateProject = async (updatedProject) => {
    try {
      ...
      await collectionsApi.updateCollection(
        updatedProject.collectionId || updatedProject.id,
        {
    ...
    if (projectToDelete) {
      try {
        ...
        await collectionsApi.deleteCollection(
          projectToDelete.collectionId || projectToDelete.id,
        );
  ...
  const handleSendPrompt = () => {
    ...
    setTimeout(() => {
      ...
      if (
        promptText.toLowerCase().includes('genera') ||
        promptText.toLowerCase().includes('crea documento')
      ) {
    ...
    try {
      ...
      for (const file of Array.from(files)) {
        try {
          ...
          const collectionId =
            selectedProject?.collectionId || selectedProject?.id;
  ...
  const handleSaveDocument = (updatedDocument) => {
    ...
    if (selectedProject) {
      setSelectedProject((prevProject) => {
        ...
        setProjects((prevProjects) =>
          prevProjects.map((p) =>
            p.id === prevProject.id ? updatedProject : p,
          ),
        );
  ...
  const handleRestoreActivity = (activity) => {
    ...
    if (selectedProject) {
      setSelectedProject((prevProject) => {
        const hasDoc = (prevProject.documents || []).some(
          (d) => d.id === activity.documentId,
        );
  ...
  const handleDeleteDocumentFromProject = async (documentId) => {
    ...
    try {
      ...
      if (selectedProject.collectionId) {
        await collectionsApi.removeDocumentFromCollection(
      ...
      setSelectedProject((prevProject) => {
        const updatedDocs = (prevProject.documents || []).filter(
      ...
      if (currentArtifact && currentArtifact.id === documentId) {
        ...
        const remainingGeneratedDocs = selectedProject.documents.filter(
          (doc) => doc.id !== documentId && doc.isGenerated,
        );
        setCurrentArtifact(
          remainingGeneratedDocs.length > 0 ? remainingGeneratedDocs[0] : null,
        );
      }
