 ...
 */
export default function LexychainPanel({ activities = [], onRestore = () => {} }) {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'thinking': return <WarningIcon color="warning" fontSize="small" />;
      case 'viewing': return <InfoIcon color="info" fontSize="small" />;
      case 'editing': return <EditIcon color="primary" fontSize="small" />;
      case 'browsing': return <CheckCircleIcon color="success" fontSize="small" />;
  ...
  const isRestorableActivity = (activity) => {
    return activity.status === 'editing' && activity.documentId && activity.version;
  };
    ...
    <>
      ...
      >
        <Box sx={{ 
          ...
          bgcolor: expanded ? 
            theme => theme.palette.mode === 'dark' ? 'rgba(0,0,0,0.1)' : 'rgba(0,0,0,0.02)' : 
            'transparent',
          cursor: 'pointer',
          '&:hover': {
                  ...
                  <React.Fragment key={idx}>
                    <ListItemButton
                      sx={{ 
                        ...
                        cursor: isRestorableActivity(act) ? 'pointer' : 'default',
                        borderLeft: act.agent ? `3px solid ${getAgentColor(act.agent)}` : 'none',
                      }}
                    ...
                    >
                      <ListItemIcon sx={{ minWidth: 32 }}>
                        {act.agent ? (
                          ...
                          >
                            {act.agent.name ? act.agent.name.charAt(0).toUpperCase() : <PersonIcon fontSize="small" />}
                          </Avatar>
                        ) : (
        ...
        {!expanded && activities.length > 0 && (
          ...
          >
            {activities[0].agent ? (
              ...
              >
                {activities[0].agent.name ? activities[0].agent.name.charAt(0).toUpperCase() : <PersonIcon fontSize="small" />}
              </Avatar>
            ) : (
            ...
            <Typography 
              ...
              sx={{ 
                ...
                color: activities[0].agent ? alpha(getAgentColor(activities[0].agent), 0.9) : 'text.primary',
                fontWeight: 500,
                flex: 1
              }}
            >
      ...
      >
        ...
        <DialogContent>
          ...
          <Typography variant="body2">
            Vuoi ripristinare questa versione del documento? Qualsiasi modifica non salvata andrà persa.
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setIsRestoreDialogOpen(false)}>Annulla</Button>
          <Button onClick={handleConfirmRestore} color="primary" variant="contained" startIcon={<RestoreIcon />}>
            Ripristina
          </Button>
        </DialogActions>
      </Dialog>
